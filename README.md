...
1. Add PostgreSQL to the same Railway project and use its `DATABASE_URL`.
2. Create the Telegram bot, Telegram API credentials, and public updates channel.
3. Promote the control bot in the updates channel so membership checks work.
4. Generate and save one Fernet key for `SESSION_ENCRYPTION_KEY`.
5. Add all environment variables, including `PUBLIC_BASE_URL`.
6. Deploy and confirm `GET /health`.
7. Configure and test the Razorpay webhook.
8. Test `/start`, channel gate, `/connect`, task validation, test payment, and forwarding with test accounts.
