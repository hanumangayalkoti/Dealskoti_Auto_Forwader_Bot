...
def _named_file(stored_file):
    """Reads the stored file into a stream that carries its ORIGINAL name.

    Telethon takes the upload file name from the stream's `.name`. A real file
    handle will not let that attribute be set, so the bytes are wrapped in a
    BytesIO, whose name can be assigned. Without this the on-disk basename was
    used and subscribers received "8844066493_a1b2c3d4_MyApp.apk" instead of
    "MyApp.apk".
    """
    path = str(stored_file["local_path"])
    original = str(stored_file["file_name"] or os.path.basename(path))
...
#
# Everything else is generated by the SERVER from the text itself — a URL, an
# @mention, a #hashtag, a phone number. Sending those back is not just
# unnecessary, it is harmful: Telegram can reject the entire entity list over
# one bad member, and the custom (premium) emoji goes down with it. Since a
# real deals post almost always contains an @username or a link, premium emoji
# were being dropped nearly every time, even for users who had Premium.
#
# MessageEntityMentionName is the worst of them: it references a user and
# needs a proper InputUser, so sending it plain is rejected outright.
#
# Nothing is lost by filtering: Telegram re-detects links, mentions and
# hashtags on its own, so they stay clickable. Only TextUrl (a link hidden
# behind words) must be sent, because the server cannot infer it.
SENDABLE_ENTITIES = (
    MessageEntityBold,
    MessageEntityItalic,
    MessageEntityUnderline,
...
def sendable_entities(entities):
    """Keeps only the entities Telegram accepts from a sender."""
    if not entities:
        return None
    kept = [e for e in entities if isinstance(e, SENDABLE_ENTITIES)]
    return kept or None
...
class ForwardingEngine:
    ...
    def build_text(
        ...
        if mono:
            ...
            if text and plan_has(plan_name, F_HIDDEN_LINKS) and settings.get("disable_hidden_links"):
                revealed = self._reveal_hidden_links(text, entities)
                if revealed != text:
                    # The text was rewritten, so the old offsets are useless.
                    text, entities = revealed, None
                # If NOTHING was revealed — the common case, since most posts
                # have no hidden link at all — the text is untouched and the
                # entities are still valid. An earlier version dropped them
                # unconditionally, which silently stripped bold, italic and
                # premium emoji from every post of anyone who had this setting
                # switched on.
        ...
        return (
            "\n\n".join(parts), None,
            sendable_entities(_shift_entities(rich.entities(), shift)),
        )
                ...
                try:
                    if not clean_copy:
                        ...
                        if replacement_file is not None and os.path.exists(
                            ...
                            #
                            # Sent through a handle carrying the ORIGINAL file
                            # name. Passing the path alone made Telethon use
                        ...
                        if isinstance(payload, io.BytesIO):
                            ...
                            # The name is copied from the STREAM being cloned,
                            # not from media_file: for a replacement file that
                            # would have overwritten the user's real file name
                            # with "photo.jpg".
    ...
    # Bot API refuses a file_id that came from a user account, so the only way
    # for the bot to post someone else's media is to hand it the actual bytes.
    # Those are already in hand for photos (the pipeline downloads them for the
    ...
    async def _send_with_buttons(
        ...
        Handles text posts and photos. Anything heavier falls back to the
        user's own account, which sends the post correctly but without
        buttons — a post that arrives beats a post that does not.

        Returns the sent message, or None if the bot could not do it.
        """
        if self.bot is None:
            return None
        try:
            ...
            async for message in iterator:
                ...
                try:
                    sent = await client.send_message(
                        ...
                        message=text,
                        file=media,
                        formatting_entities=sendable_entities(
